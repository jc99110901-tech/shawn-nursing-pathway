...
| 平台 | 最低门槛方式 | 能否保持自动更新 |
|---|---|---|
| Codex | 安装完整 `skills/` 套件或 Suite ZIP | 本地需重新拉取或更新仓库 |
...
- [`SNP Skill Suite ZIP`](../dist/shawn-nursing-pathway-suite.zip)：包含主路由和 7 个子 Skill，适合 Codex 或明确支持多 Skill 目录的运行环境。
...
- Codex 可直接安装 `SNP Skill` 主路由和 7 个子 Skill。
- 不支持多 Skill 路由的平台使用 Lite／Full 合并版，功能仍按同一模块流程执行。
- WorkBuddy ZIP 保持一个根 `SKILL.md`，子模块作为合并包内参考流程，不声称平台原生启动了 7 个独立 Skill。
